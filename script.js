...
// =====================
// Cielo estrellado + Constelación de Orión (scroll-in / scroll-out)
// =====================
document.addEventListener("DOMContentLoaded", () => {
  ...
  const scaleX = 0.7;   // más delgada
  const scaleY = 0.85;  // altura ajustada
  const offsetX = 0.08; // mover hacia la izquierda/derecha
  const offsetY = 0.0;  // mover arriba/abajo
  ...
  // ---- Estado de la constelación (alpha en función del scroll)
  let orionAlpha = 0;
  function updateOrionAlpha() {
    const target = Math.max(0, Math.min(1, window.scrollY / 250));
    orionAlpha += (target - orionAlpha) * 0.08;
    if (heroContent) heroContent.style.opacity = (1 - orionAlpha).toFixed(3);
  }
  ...
  function animate() {
    ...
    if (canvas.width < 768) { // móvil
      logicalSize = canvas.height * 0.75; // usa casi toda la altura
    }
